...
void Quadtree::colorPick(float elevate, Quadtree *t, float *depthColor, int depthColorLen)
{
    if(t->boundary2)
    {
        ...
        std::cout << "func has been work here!" << std::endl;
...
void display(){
    ...
    std::shared_ptr<BoundaryBox> BB_init2(new BoundaryBox(BB_centerx, BB_centery, BB_dim));
...
// insert a point
    ...
    pt2d *insertPt2 = new pt2d(5, 7);
    pt2d *insertPt3 = new pt2d(10, 14);
    pt2d *insertPt4 = new pt2d(11, 7);

    quadtreeTest->insert(*insertPt);
    quadtreeTest->insert(*insertPt2);
    quadtreeTest->insert(*insertPt3);
    quadtreeTest->insert(*insertPt4);

    //int result = quadtreeTest->count_nodes(quadtreeTest);

    int tree_count_elements = quadtreeTest->count_elements(quadtreeTest);

    std::cout << tree_count_elements << "\n";*/
